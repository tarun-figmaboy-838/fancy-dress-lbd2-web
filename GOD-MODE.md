...
>
> The rest of this document is the **Bubble Days** specification it was written
> for. The layer that actually ships in this repo lives in [`god-mode/`](god-mode/)
> and follows the same architecture — utils loaded first, a fetched panel template
> with an inline fallback, `Shift + G` to toggle, and a complete teardown on the
> way out — adapted to this game's `Engine` / `Controllers` API.
>
> **What is implemented** (focused on the alignment brief: edit x and y like
> Figma, and drag):
>
> | File | Role |
> |---|---|
> | `god-mode/god-mode-utils.js` | `window.GodModeUtils` — design-grid maths, clipboard, visibility, `activeRoot()`, toasts |
> | `god-mode/god-mode-live-editor.js` | `window.GodModeLiveEditor` — the Figma-style editor: pick, drag, 8 resize handles, arrow-key nudge, snap, copy/export, save/load, exact reset |
> | `god-mode/god-mode-qa.js` | `window.GodModeQA` — smoke, asset, placement, scale-mechanics and layout/overflow checks |
> | `god-mode/god-mode-panel.html` | panel template, fetched at runtime |
> | `god-mode/god-mode.js` | `window.FancyDressGodMode` — activation, shortcuts, scene flow, visual debug |
> | `god-mode/god-mode.css` | all styles, `position: fixed` outside the scaled stage |
>
> **Coordinates.** Two systems are shown side by side for every element:
> **X / Y / W / H** in design-grid pixels (1920×1080, top-left origin, y **down**) —
> what a designer reads off a Figma frame — and **aPos X / Y**, the Unity
> `anchoredPos` for the same node (y **up**, pivot relative), which is what goes
> back into the scene. Editing either updates the other. Drag deltas are divided
> by the letterbox scale, so dragging is 1:1 on the scaled stage.
>
> **Registered elements** are discovered by walking the active level's live tree
> rather than from a hard-coded selector table, so the same code covers the
> Tutorial and all six levels — node names repeat across levels, and only the one
> on screen is offered.
>
> **Not implemented**, because this game has no equivalent: the rewards/gems and
> timer sections, the Animation Ideas bar, and the kid-focused UX-review module.
> The Live Layout Editor's group-mode/single-target split is also collapsed —
> every pick here is a single target, which is what per-element alignment needs.
>
> Shortcuts: `Shift+G` toggle · arrows nudge 1px · `Shift`+arrows 10px ·
> `Ctrl/Cmd+C` copy values · `Ctrl/Cmd+E` copy patch · `N`/`P` level · `R` restart ·
> `D` drop item · `F` fill correct · `C` check · `H` show guide · `B` bounds ·
> `S` safe area · `G` grid · `Q` run QA · `1`–`4` animation speed.
>
> ---

## Original specification (Bubble Days)
